...
lv_obj_t *create_version_app(void)
{
    ...
    lv_obj_remove_style(version_app->version_page, NULL, LV_PART_SCROLLBAR);
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        lv_obj_set_size(cont, 240, 50);                                    // 设置容器大小
        lv_obj_align(cont, LV_ALIGN_TOP_MID, 0, 60 + i * 50);              // 调整位置
